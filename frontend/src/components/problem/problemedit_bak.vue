    ...
    <el-col :span="18">
      <el-row>
        <el-card shadow="always">
          ...
          <el-row :gutter="18" class="problem-description-title">Level </el-row>
          <el-row :gutter="18" class="problem-descriptions">
            ...
            <el-container style="height: 500px; border: 1px solid #eee">
              ...
              >
                <el-menu :default-openeds="['1', '3']">
                  <el-submenu index="1">
                    <template slot="title"
                      ><i class="el-icon-message"></i>代码文件</template
                    >
                    <el-menu-item-group>
                      <template slot="title">模板文件</template>
                      <li
                        v-for="(code_item, index) in code_items"
                        :key="code_item.code"
                      >
                        <el-menu-item
                          @click="
                            code_select(code_item.code, 'verilog', 0, index)
                          "
                          >code.v
                        </el-menu-item>
                      </li>
                      <li
                        v-for="(code_template, index) in code_templates"
                        :key="code_template.code"
                      >
                        <el-menu-item
                          @click="
                            code_select(code_template.code, 'verilog', 1, index)
                          "
                          >code_template.v</el-menu-item
                        >
                      </li>
                    </el-menu-item-group>
                  ...
                  <el-submenu index="2">
                    <template slot="title"
                      ><i class="el-icon-menu"></i>判题文件</template
                    >
                    <li
                    ...
                    >
                      <el-menu-item-group>
                        ...
                        <el-menu-item
                          @click="
                            code_select(testcase.code[0], 'python', 2, index)
                          "
                          >wavedump.py</el-menu-item
                        >
                        <el-menu-item
                          @click="
                            code_select(testcase.code[1], 'python', 3, index)
                          "
                          >vcd_main.py</el-menu-item
                        >
                        <el-menu-item
                          @click="
                            code_select(testcase.code[2], 'verilog', 4, index)
                          "
                          >testbench.v</el-menu-item
                        >
                        <el-menu-item
                          @click="
                            code_select(testcase.code[3], 'python', 5, index)
                          "
                          >vcd_visualize.py</el-menu-item
                        >
                        <el-menu-item
                          @click="
                            code_select(testcase.code[4], 'shell', 6, index)
                          "
                          >main.sh</el-menu-item
                        >
                      </el-menu-item-group>
                    </li>
                  </el-submenu>
    ...
    if (this.is_change) {
      ...
      this.$axios
        ...
        .then((response) => {
            ...
            for (var i = 0; i < testcases.length; i++) {
              this.$axios
                ...
                .then((response) => {
                  ...
                  switch (
                    filename //对应顺序为wavedump.py,vcd_main.py,testbench.v,vcd_visualize.py,main.sh
                  ) {
                    case "wavedump.py":
          ...
          if (template != null) {
            this.$axios.get("/files/" + template + "/").then((response) => {
          ...
          if (judge != null) {
            this.$axios.get("/files/" + judge + "/").then((response) => {
        ...
        .catch((error) => {
          this.$message.error(
            "发生错误！" + "(" + JSON.stringify(error.response.data) + ")"
          );
        });
  ...
  */
    ...
    async upload_testcase(code_ids, problemid) {
      ...
      return this.$axios.post("/problem-testcases/", body).catch((error) => {
        this.$message.error(
          "提交错误！" + "(" + JSON.stringify(error.response.data) + ")"
        );
      });
    },
    ...
    async gettestcase(problemid) {
      return this.$axios
        .get("/problem-testcases/?problem=" + problemid + "/")
        .catch((error) => {
          this.$message.error(
            "题目错误！" + "(" + JSON.stringify(error.response.data) + ")"
          );
        });
    },
    ...
    async problemedit() {
      if (this.is_edit == false) {
        ...
        for (var i = 0; i < this.testcases.length; i = i + 1) {
          const wavedump_id = await this.upload(
            this.testcases[0].code[0],
            "wavedump.py"
          );
          const vcd_main_id = await this.upload(
            this.testcases[0].code[1],
            "vcd_main.py"
          );
          const testbench_id = await this.upload(
            this.testcases[0].code[2],
            "testbench.v"
          );
          const vcd_visualize_id = await this.upload(
            this.testcases[0].code[3],
            "vcd_visualize.py"
          );
          const main_id = await this.upload(
            this.testcases[0].code[4],
            "main.sh"
          );
        ...
        const code_id = await this.upload(
          this.code_items[0].code,
          "code_ref.v"
        );
        const template_id = await this.upload(
          this.code_templates[0].code,
          "template_code.v"
        );
        var body = {};
        ...
        if (this.have_ddl) body["deadline_time"] = this.ddl_time;
        console.log(template_id);
        if (this.is_change) {
          this.$axios.patch(
            "/problems/" + this.$route.params.problemid + "/",
            body
          );
          var testcase_list = await this.gettestcase(
            this.testcases[0].code[0],
            "wavedump.py"
          );
          ...
          for (var j = 0; j < testcase_list.length; j = j + 1) {
            this.$axios
              .delete("/problem-testcases/" + testcase_list[j].id + "/")
              .catch((error) => {
                this.$message.error(
                  "提交错误！" + "(" + JSON.stringify(error.response.data) + ")"
                );
