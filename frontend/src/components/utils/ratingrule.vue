<template>
  <div>
    ...
    <h4>
      2、每场比赛后计算一个排名得分S （ S=（比赛得分线性归一化后）*比赛队伍数
      ）（线性归一化函数: x = (x-min)/(max-min)）
    </h4>
    ...
    <h3>LPOJ个人赛：</h3>
    <h4>
      每道题目有一个积分，每分钟衰减(0.7/i)%，i为题号
      1~26,比赛一般为120分钟。每道题分数按顺序为1000 1500 1600 2000 2000
      2000往后都为2000
    </h4>
    ...
    <br />
    <h2>
      每场比赛结果都会保存，所以如果积分系统崩了的话，可以用其他方式计算，保证公平性
    </h2>
  </div>
</template>
