...
if [ -f "backup-files-volume-${BACKUP_SUFFIX}.tar" ] && [ -f "backup-db-volume-${BACKUP_SUFFIX}.tar" ]; then
    docker run -it --rm --name "VLab-Restore-Files" -v "$(pwd):/backup" -v verilog-oj_files-volume:/volume ubuntu:20.04 tar xvf "/backup/backup-files-volume-${BACKUP_SUFFIX}.tar" -C /volume
